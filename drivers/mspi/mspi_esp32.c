...
{
	if (clock_frequency > MSPI_MAX_FREQ) {
				 ...
				 const struct mspi_dev_cfg *mspi_dev_config)
{
    ...
    if (cfg_mask & MSPI_DEVICE_CONFIG_FREQUENCY) {
        int ret = update_timing_config(config, data, &data->hal_dev_config.timing_conf, mspi_dev_config->freq);
...
static int mspi_esp32_config(const struct mspi_dt_spec *spec)
{
	...
	ret = update_timing_config(config, data, &data->hal_dev_config.timing_conf, data->clock_frequency);
...
static int mspi_esp32_init(const struct device *dev)
{
    ...
    data->clock_frequency = config->clock_frequency;
