...
static int cs_gpio_set(
	const struct mspi_esp32_data *data,
	const struct mspi_esp32_config *config,
	bool active)
{
	...
	const struct gpio_dt_spec *cs =
		config->mspi_config.ce_group + data->mspi_dev_config.ce_num;

	bool pin_high = (data->mspi_dev_config.ce_polarity == MSPI_CE_ACTIVE_HIGH)
		? active
		: !active;
...
static int update_timing_config(
	const struct mspi_esp32_config *config,
	...
	uint32_t clock_frequency)
{
	if (clock_frequency > config->mspi_config.max_freq) {
		LOG_ERR("Requested freq %u exceeds max %u",
			clock_frequency, config->mspi_config.max_freq);
		return -EINVAL;
	}
	...
	LOG_DBG("timing: clk_src_hz=%u expected=%u duty=%u input_delay_ns=%u use_gpio=%d",
		tp.clk_src_hz, tp.expected_freq, tp.duty_cycle,
		tp.input_delay_ns, tp.use_gpio);
...
static int mspi_gdma_config(const struct device *dev, uint8_t dir, uint8_t *buf, size_t len)
{
	...
	LOG_DBG("gdma_config: dir=%d ch=%u buf=%p len=%u slot=%d",
		dir, dma_channel, buf, len, cfg->dma_host);
...
static void transfer_setup_cmd_addr(
 ...
 */
static uint8_t *transfer_prepare_data(
	...
	int *ret)
{
	...
	*ret = 0;
	...
	if (packet->dir == MSPI_TX) {
		...
		bool need_bounce = config->dma_enabled ||
			!esp_ptr_dma_capable((uint32_t *)dst) ||
			((uintptr_t)dst % 4 != 0) ||
			(dma_len % 4 != 0);
...
static int transfer_start_gdma(
	const struct device *dev,
...
static int transfer_start_intdma(
	const struct device *dev,
	spi_hal_context_t *hal,
	const spi_hal_dev_config_t *hal_dev,
	spi_hal_trans_config_t *tc,
	size_t dma_buf_len)
{
	...
	if (tc->rcv_buffer) {
		LOG_DBG("transfer: intDMA RX start buf=%p len=%zu",
...
#endif
	if (tc->send_buffer) {
		LOG_DBG("transfer: intDMA TX start buf=%p len=%zu",
			tc->send_buffer, dma_buf_len);
		mspi_dma_tx_start(dev, (uint8_t *)tc->send_buffer, dma_buf_len);
	}

	LOG_DBG("transfer: intDMA enable_data_line tx=%d rx=%d",
		tc->send_buffer != NULL, tc->rcv_buffer != NULL);
	spi_hal_enable_data_line(hal->hw,
		tc->send_buffer != NULL,
		tc->rcv_buffer != NULL);
...
static void transfer_post_gdma(
	const struct device *dev,
	spi_hal_trans_config_t *tc,
	size_t dma_buf_len)
{
	...
	size_t packet_index)
{
	...
	LOG_DBG("transfer[%zu]: success", packet_index);
...
static int IRAM_ATTR update_transfer_config(const struct device *dev)
{
	...
	LOG_DBG("update_transfer_config: mode=%d cs_pin_id=%d positive_cs=%d "
		"half_duplex=%d dummy_bits=%u io_mode=%d "
		"lines(cmd=%u addr=%u data=%u)",
		hal_dev->mode, hal_dev->cs_pin_id, hal_dev->positive_cs,
		hal_dev->half_duplex, data->trans_config.dummy_bits,
		data->mspi_dev_config.io_mode,
		data->trans_config.line_mode.cmd_lines,
...
static int mspi_esp32_dev_config(
	const struct device *dev,
	...
	const struct mspi_dev_cfg *mspi_dev_config)
{
	...
	if (cfg_mask & ~supported) {
		LOG_ERR("Unsupported config mask bits 0x%x",
			cfg_mask & ~supported);
		return -ENOTSUP;
	}
	...
	if (cfg_mask & MSPI_DEVICE_CONFIG_FREQUENCY) {
		int ret = update_timing_config(
			config,
			data,
			&data->dev_config.timing_conf,
...
static int IRAM_ATTR mspi_esp32_transceive(
	const struct device *dev,
	[[maybe_unused]] const struct mspi_dev_id *dev_id,
	const struct mspi_xfer *xfer)
{
...
static int init_dma(const struct device *dev)
{
	...
	LOG_DBG("init_dma: GDMA dma_dev ready, tx_ch=%u rx_ch=%u slot=%d",
	...
	LOG_DBG("init_dma: integrated DMA path, enabling DMA clock");
	if (clock_control_on(config->clock_dev,
		(clock_control_subsys_t)config->dma_clk_src)) {
		LOG_ERR("Could not enable DMA clock");
		return -EIO;
	}
	data->hal.dma_enabled = true;
	LOG_DBG("init_dma: DMA clock enabled, dma_host=%d", config->dma_host);
	...
	DPORT_SET_PERI_REG_BITS(
		...
		config->dma_host + 1,
		(config->dma_host + 1) * 2);
	LOG_DBG("init_dma: ESP32 DMA channel wired");
#endif

#endif /* SOC_GDMA_SUPPORTED */

	LOG_DBG("init_dma: done");
	return 0;
}
...
static int cs_configure(const struct device *dev)
{
	...
	for (int i = 0; i < mspi_config->num_ce_gpios; ++i) {
		...
		if (!device_is_ready(cs->port)) {
			LOG_ERR("CS GPIO %s pin %d not ready",
				cs->port->name, cs->pin);
			return -ENODEV;
		}
		...
		int ret = gpio_pin_configure_dt(
			cs,
			pin_high ? GPIO_OUTPUT_ACTIVE : GPIO_OUTPUT_INACTIVE);
...
static int mspi_esp32_config(const struct mspi_dt_spec *spec)
{
	...
	LOG_INF("ESP32 MSPI (peripheral %u) configured at %u Hz",
		config->peripheral_id, config->mspi_config.max_freq);
...
static int mspi_esp32_init(const struct device *dev)
{
	...
	data->dev_config.half_duplex =
		(config->mspi_config.duplex == MSPI_HALF_DUPLEX);
	...
	data->dev_config.cs_pin_id =
		(config->mspi_config.num_ce_gpios > 0) ? -1 : 0;
...
#define ESP32_MSPI_INIT(inst)											\
	PINCTRL_DT_INST_DEFINE(inst);                                      	\
                                                                        \
	static struct gpio_dt_spec ce_gpios##inst[] =                      	\
	...
	static const struct mspi_esp32_config mspi_esp32_cfg_##inst = {    	\
		...
		.clock_source  = DT_ENUM_IDX_OR(DT_DRV_INST(inst),       		\
